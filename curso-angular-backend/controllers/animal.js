...
function getAnimal(req, res){
    ...
    Animal.find({_id: animalId}).populate({path: 'user'}).exec((err,animals)=>{
        if(err){
            res.status(500).send({message: 'Error en la petición'});
        }else{
            if(!animals){
                res.status(400).send({message: 'El animal no existe'});
            }else{
            res.status(200).send({animals});
            }
        }
...
function uploadImage(req, res){
    ...
    if(req.files){
        ...
        if(file_ext == 'png' || file_ext == 'jpg' || file_ext == 'jpeg' || file_ext == 'gif'){
            ...
            Animal.findByIdAndUpdate(animalId, {image: file_name}, {new:true}, (err, unimalUpdated) =>{
                if(err){
                    res.status(500).send({
                        message: 'Error actualizar animal'
                    });
...
function deleteAnimal(req, res){
    ...
    Animal.findByIdAndRemove(animalId, (err, animalUpdated) =>{
        if(err){
            res.status(500).send({
                message: 'Error eliminar animal'
            });
        }else{ 
            if(!animalUpdated){
                res.status(404).send({
                    message: 'No se ha podido eliminar animal'
                });
            }else{
                res.status(200).send({
                    animal: animalUpdated,
